...
def getTopoLoss(stu_tensor, tea_tensor, topo_size=100, pd_threshold=0.7, loss_mode="mse"):
    ...
    stu_tensor = stu_tensor.squeeze()
    tea_tensor = tea_tensor.squeeze()

    if stu_tensor.ndim != 2 or tea_tensor.ndim != 2:
        raise ValueError(f"getTopoLoss expects 2D [H,W], got {stu_tensor.shape} and {tea_tensor.shape}")
